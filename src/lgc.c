...
static void linkgclist_(GCObject *o, GCObject **pnext, GCObject **list) // 将一个对象挂在list链上，位于链头，然后将对象设置为灰色
{
  lua_assert(!isgray(o)); /* cannot be in a gray list */
  *pnext = *list;         // obj->gclist = g->gray
  *list = o;              // g->gray = obj
...
static lu_mem traversetable(global_State *g, Table *h)
{
  ...
  if (mode && ttisstring(mode) && /* is there a weak mode? */
      (cast_void(weakkey = strchr(svalue(mode), 'k')), cast_void(weakvalue = strchr(svalue(mode), 'v')), (weakkey || weakvalue)))
  {               /* is really weak? */
...
static lu_mem propagatemark(global_State *g)
{
  GCObject *o = g->gray;
  nw2black(o);
  g->gray = *getgclist(o); /* remove o from 'gray' list */ // 将o移出链变，并将链表头部指向o->gclist所指向的下一个元素
  switch (o->tt)
  {
    case LUA_VTABLE:
